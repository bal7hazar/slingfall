...
| `src/main.ts` | wiring |
| `src/vm/` | the cairo-vm worker: chunk sizing rule, chunk loop, worker, `VmClient`, `WorkerTraceSource` (lot G1c) |
| `vm/` | the Rust runner compiled to wasm, its build scripts and fixtures ([`vm/README.md`](vm/README.md)) |
...
ticks from the worker as `TraceFrame`s, parsed by a `TickLineParser` (`src/trace/lines.ts`); the
caller passes the `TraceLevel`. The app build never needs the wasm: the worker imports it at run
time. The worker has only been tested in Node (in-process), not in a browser.

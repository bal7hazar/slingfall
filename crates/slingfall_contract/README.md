...
| `submit` | the contract: `ISlingfall` (`register_level`, `set_level_active`, `level`, `level_data`, `simulate`, `submit`, `submit_settled`, `attempt`, `best`, `leaderboard`), `ISlingfallAdmin` (`admin`, `set_admin`, `set_virtual_os_hash`, `set_sim_class_hash`, `set_verifier`, `set_attestation_key` and their reads), `ISlingfallSatellite` (`satellite_config`, `set_satellite_config`); `nullifier::{NONE, ATTESTED, SETTLED}`; events `LevelRegistered`, `LevelActiveSet`, `LevelValidated`; `submit::errors` (panic messages) |
| `verifier` | `Verifier<T> { fn check(ref self, claim: Outputs, evidence: Span<felt252>) -> bool }`; `Snip36Verifier` (`tx_info.proof_facts`: program hash at `PROGRAM_HASH_INDEX` equal to the admin's `virtual_os_hash`, and `message_hash(this, MARKER, outputs felts)` among the following facts); `StubVerifier` (`evidence = [r, s]`, Stark ECDSA over `attestation_hash = poseidon(outputs felts)`); `SatelliteVerifier` (`evidence = args`, `c1main`'s argument: the Atlantic fact of the run on Herodotus's Satellite, translated or bridged keccak; `SatelliteConfig`, `ISatellite`, `atlantic_output`, `integrity_fact`, `sharp_fact`, `run_args`; `docs/proving.md` "Contract side (E3b)"); `VerifierKind` (`Snip36`, `Stub`, `Satellite`) |
...
Two tiers (lot E3b): `submit` with `Stub` / `Snip36` records a *provisional* attempt (nullifier
`ATTESTED`); `submit_settled(outputs, args)` (or `submit` with `verifier = Satellite`) records a
*settled* one (`SETTLED`), and may settle an `ATTESTED` attempt once (its record, when it is the
player's best, becomes `settled`). `Record.settled` and `LevelValidated.settled` carry the tier.

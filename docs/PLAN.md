...
| 5 | E3b | settled validation: `SatelliteVerifier` (fact recomputed from calldata, Satellite view call), prover service (Atlantic with translation), client provisional / settled status, **Sepolia deployment with the owner's account and the first settled `submit` = M5** | Opus | E3a, P1b |

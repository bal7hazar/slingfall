...
| 5 | P1b | sound chunked proofs: public binding headers (`LEVEL_HASH`, `STATE_IN_HASH`, `INPUTS_HASH`) on `init` / `step_chunk` / `outputs`, `verify.py` link checks from public data, local measurements on the VPS | Opus | P1 |
...
| 5 | E3a | Atlantic (Stone) + Integrity round trip on Sepolia with `main` on `one_block-miss` then `pile10-reference`: lanes / layouts, latencies, the exact fact formula, the E3b contract spec (owner's Atlantic key + Sepolia account in the executor's env; read-only on-chain) | Opus | P1 |

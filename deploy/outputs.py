...
and writes `{"case", "player", "outputs": [10 x 0x felts], "steps"}`. The simulation never reads
the player: every felt but `player` and `inputs_hash` must equal the golden's, which is checked.
"""
...
def replay(case_name: str, player: int, build: bool) -> dict:
    ...
    return {"case": case_name, "player": hex(player), "outputs": [hex(v) for v in outputs], "steps": steps}
...
def main(argv: list[str]) -> int:
    ...
    doc = replay(args.case, int(args.player, 0), not args.no_build)
